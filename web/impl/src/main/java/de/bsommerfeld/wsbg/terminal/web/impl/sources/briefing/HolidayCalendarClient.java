...
public class HolidayCalendarClient {
    ...
    /** Both holiday APIs are wall-less — direct-first, the browser is the rescue. */
    ...
    @Inject
    public HolidayCalendarClient(WebFetcher fetcher) {
    ...
    private <T> List<T> fetchJson(String url, java.util.function.Function<String, List<T>> parser) {
        try {
            WebResponse resp = fetcher.fetch(url, Map.of("Accept", "application/json"),
