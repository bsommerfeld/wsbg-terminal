...
final class ChunkedDownload {
    ...
    */
    ...
    /** The server answered a Range request with the whole file: it cannot serve chunks. */
    static final class RangeIgnoredException extends IOException {
        RangeIgnoredException(String url) {
            super("Server ignores Range requests: " + url);
        }
    }
        ...
        try (ExecutorService pool = Executors.newFixedThreadPool(connections)) {
            ...
            for (Future<Void> f : futures) {
                try {
                    ...
                    */
                    pool.shutdownNow();
                    if (e.getCause() instanceof RangeIgnoredException rangeIgnored) {
                        throw rangeIgnored;
                    }
    ...
    private static void downloadChunk(String url, byte[] target,
        ...
        try {
            ...
            */
             ...
             * header answers 200 with the entire file instead. That body cannot
             * go into this chunk's slice - it would run past the array's end -
             * so the chunked path gives up and the caller takes one connection.
            */
            int status = response.statusCode();
            if (status == 200) {
                response.body().close();
                throw new RangeIgnoredException(url);
            }
            if (status != 206) {
