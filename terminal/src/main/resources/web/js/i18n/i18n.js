...
const DICT = {
  de: {
    ...
    'settings.llm.title': 'KI-Backend',
    'settings.llm.backend.name': 'Modell-Quelle',
    'settings.llm.backend.hint': 'Lokales Modell oder eigener API-Key (kein Download nötig).',
    'settings.llm.backend.more': 'Lokal lädt einmalig ~15 GB Modelle herunter. Mit eigenem API-Key läuft alles über deinen Anbieter — nichts wird installiert. Änderungen greifen nach einem Neustart.',
    'settings.llm.backend.ollama': 'Lokal (Ollama)',
    'settings.llm.backend.openai': 'Eigener API-Key (OpenAI-kompatibel)',
    'settings.llm.backend.anthropic': 'Eigener API-Key (Anthropic / Claude)',
    'settings.llm.apikey.name': 'API-Key',
    'settings.llm.apikey.hint': 'Wird nur lokal in der Konfiguration gespeichert.',
    'settings.llm.baseurl.name': 'Base-URL',
    'settings.llm.baseurl.hint': 'OpenAI · Vercel AI Gateway · OpenRouter · lokaler Server.',
    'settings.llm.chatmodel.name': 'Modell (Text + Vision)',
    'settings.llm.chatmodel.hint': 'z. B. gpt-4o-mini oder claude-sonnet-4-5.',
    'settings.llm.embedmodel.name': 'Embedding-Modell',
    'settings.llm.embedmodel.hint': 'Fürs Clustering. OpenAI-kompatibel.',
    'settings.llm.embedmodel.more': 'Claude hat keine Embedding-API. Bei Anthropic optional unten einen OpenAI-kompatiblen Embed-Endpunkt setzen — sonst wird lokal (Ollama) eingebettet.',
    'settings.llm.embedbaseurl.name': 'Embedding Base-URL',
    'settings.llm.embedbaseurl.hint': 'Optional. Leer = Embeddings lokal (Ollama).',
    'settings.llm.embedapikey.name': 'Embedding API-Key',
    'settings.llm.embedapikey.hint': 'Optional. Leer = derselbe wie oben.',
  ...
  en: {
    ...
    'settings.llm.title': 'AI backend',
    'settings.llm.backend.name': 'Model source',
    'settings.llm.backend.hint': 'Local model, or your own API key (no download needed).',
    'settings.llm.backend.more': 'Local downloads ~15 GB of models once. With your own API key everything runs through your provider — nothing is installed. Changes take effect after a restart.',
    'settings.llm.backend.ollama': 'Local (Ollama)',
    'settings.llm.backend.openai': 'Your own API key (OpenAI-compatible)',
    'settings.llm.backend.anthropic': 'Your own API key (Anthropic / Claude)',
    'settings.llm.apikey.name': 'API key',
    'settings.llm.apikey.hint': 'Stored only locally in your config.',
    'settings.llm.baseurl.name': 'Base URL',
    'settings.llm.baseurl.hint': 'OpenAI · Vercel AI Gateway · OpenRouter · local server.',
    'settings.llm.chatmodel.name': 'Model (text + vision)',
    'settings.llm.chatmodel.hint': 'e.g. gpt-4o-mini or claude-sonnet-4-5.',
    'settings.llm.embedmodel.name': 'Embedding model',
    'settings.llm.embedmodel.hint': 'For clustering. OpenAI-compatible.',
    'settings.llm.embedmodel.more': 'Claude has no embeddings API. For Anthropic, optionally set an OpenAI-compatible embed endpoint below — otherwise embeddings run locally (Ollama).',
    'settings.llm.embedbaseurl.name': 'Embedding base URL',
    'settings.llm.embedbaseurl.hint': 'Optional. Empty = embeddings run locally (Ollama).',
    'settings.llm.embedapikey.name': 'Embedding API key',
    'settings.llm.embedapikey.hint': 'Optional. Empty = same as above.',
