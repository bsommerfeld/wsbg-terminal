...
> ```bash
> grep -rEho 'https?://[A-Za-z0-9._-]+' --include='*.java' */src/main */*/src/main | sort -u
...
| KI-Modellserver, eigene isolierte Instanz | `127.0.0.1:11500` | agent · OllamaServerManager, ChatModelFactory |
| KI-Modellserver, externer Endpunkt | benutzerdefiniert (`agent.endpoint-url`) | agent · ChatModelFactory; updater · EndpointProbe |
...
- **Der externe KI-Endpunkt hat keinen festen Host:** Steht `agent.endpoint-mode` auf `remote`, gehen alle Modell-Aufrufe an die Adresse aus `agent.endpoint-url` - üblicherweise eine Maschine im eigenen Netz, je nach `agent.endpoint-api` über Ollamas Schnittstelle (`/api/chat`) oder die von OpenAI (`/v1/chat/completions`). Im Quelltext steht keine URL, der Prüfbefehl findet sie also nie; sie zählt trotzdem und steht darum als eigene Zeile im Register. Im Auslieferungszustand (`managed`) verlässt kein KI-Aufruf die Maschine.
