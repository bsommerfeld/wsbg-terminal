 ...
 * In production this rides the shared {@link WebFetcher} (browser joker →
 * direct), so the request carries a real browser TLS fingerprint + cookies and
 * clears the wall — exactly like the Reddit/Yahoo calls. The transport sets a
 * browser-shaped User-Agent centrally regardless.
 *
...
public class FearGreedClient {
    ...
    /** Production: rides the shared browser-joker {@link WebFetcher}. */
    @Inject
    ...
    public java.util.List<DailyScore> historySince(java.time.LocalDate since) {
        try {
            ...
            // client (live-pinned 2026-07-14; the UA now comes from the
            // transport) — the history back-fill must not depend on the
            // browser joker being up.
            WebResponse resp = fetcher.fetch(URL + "/" + since,
                    Map.of("Accept", "application/json",
                            "Referer", "https://edition.cnn.com/"),
                    java.time.Duration.ofSeconds(25), MODES);
            if (resp.status() != 200) {
