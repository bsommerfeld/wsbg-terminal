...
import de.bsommerfeld.tinyupdate.update.VersionFile;
import de.bsommerfeld.tinyupdate.update.ZipExtractor;
...
public final class TinyUpdateClient implements UpdateClient {
    ...
    */
    ...
    public TinyUpdateClient(GitHubRepository repository, Path appDirectory, ReleaseChannel channel) {
        ...
        /*
         * Through ReleaseAssetNames rather than literals: it holds the one
         * naming rule the packaging action applies too, so no caller can drift
         * away from what CI actually uploads.
        */
        this(repository, appDirectory, channel, ReleaseAssetNames.DEFAULT);
    }

    /**
     * A stream named by {@link ReleaseAssetNames} - typically
     * {@code ReleaseAssetNames.of(prefix, Platform.current())}, the stream of
     * one application for the platform this JVM runs on.
     */
    public TinyUpdateClient(GitHubRepository repository, Path appDirectory, ReleaseChannel channel,
            ReleaseAssetNames assets) {
        this(repository, appDirectory, channel,
                assets.manifest(), assets.archive(), assets.appArchive(), assets.depsArchive());
    }
