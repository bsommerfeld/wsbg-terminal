package de.bsommerfeld.wsbg.terminal.web.impl.sources.bundesanzeiger;
 ...
 *
 * <p><b>Transport carve-out (documented, deliberate):</b> the two-request Wicket
 * flow needs the {@code jsessionid} cookie to SURVIVE between the entry request
 * and the CSV request, and the house {@code WebFetcher} seam is stateless by
 * contract. This client therefore keeps its own stateful JDK {@link HttpClient}
 * with a {@link CookieManager} — the ONE implementation of the flow (the former
 * parallel seam-first attempt is gone; it only ever succeeded on the browser
 * leg and answered HTML on the plain leg). It still sends a browser User-Agent
 ...
 */
@Singleton
public class BundesanzeigerShortInterestSource implements ShortInterestSource {

    private static final Logger LOG = LoggerFactory.getLogger(BundesanzeigerShortInterestSource.class);
    ...
    @Inject
    public BundesanzeigerShortInterestSource() {
    ...
    private Register freshRegister() {
        ...
        synchronized (refreshLock) {
            ...
            if (reg != null && now - reg.fetchedAtEpochSeconds() < CACHE_TTL.toSeconds()) return reg;
            Register fetched = fetchViaCookieFlow();
