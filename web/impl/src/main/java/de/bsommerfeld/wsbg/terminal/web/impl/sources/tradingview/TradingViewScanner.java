 ...
 * Origin/Referer pair this host answers 403. The POST rides the house
 * {@link WebFetcher#post} seam (the old private JDK client is gone; the seam
 * carries bodies now), direct-first like the other API hosts of this package.
 *
...
public class TradingViewScanner {
    ...
    /** Direct-first: keyless and wall-free with the Origin/Referer pair set. */
    ...
    @Inject
    public TradingViewScanner(WebFetcher fetcher) {
    ...
    /**
     * The POST over the house seam. Returns the response body for a 200,
     * {@code null} for any other status or a transport failure - never throws
     * at the caller.
     */
    private String post(String url, String jsonBody) {
        try {
            WebResponse resp = fetcher.post(url, TradingViewApi.headers(), jsonBody,
                    "application/json", requestTimeout, MODES);
            if (resp != null && resp.status() == 200) return resp.body();
            LOG.debug("TradingView scanner answered status {}",
