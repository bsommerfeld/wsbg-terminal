 ...
 *       practice — relied on by yfinance and dozens of mirrors. The transport
 *       sets a browser-shaped User-Agent centrally to avoid the bot block.
 *       <p>
...
public class EurUsdClient {
    ...
    /**
     * Browser joker first, direct as the per-request fallback — same order the
     * editorial Yahoo calls ride, so the EUR/USD quote carries a real browser
     * fingerprint and gets a live Yahoo {@code v8/chart} answer instead of the
     * bare-client 429 that forced the Frankfurter fallback. The Frankfurter
     * floor still stands when Yahoo is genuinely down.
     */
