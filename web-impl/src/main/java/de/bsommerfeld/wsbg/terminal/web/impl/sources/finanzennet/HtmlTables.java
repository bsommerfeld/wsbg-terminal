...
final class HtmlTables {
    ...
    static Table find(List<Table> tables, String... needles) {
        for (Table t : tables) {
            String h = lower(t.heading());
            boolean all = true;
            for (String n : needles) {
                if (!h.contains(lower(n))) { all = false; break; }
            }
            if (all) return t;
        }
        return null;
    ...
    /**
     * Lower-cases with the root locale - Turkish-I safe. (Moved here from the
     * retired {@code FinanzenNetHttp}; the Akamai identity headers themselves
     * are the central transport's job now.)
     */
    ...
    private static String normalizeLabel(String s) {
        return lower(s == null ? "" : s).replaceAll("\\s+", " ").trim();
    }
}
