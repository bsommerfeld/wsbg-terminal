...
/**
 * The cheap instrument resolver behind Börsenmedien AG - <b>not</b> a news
 * source: it answers identity and price, never headlines.
 *
 ...
 */
@Singleton
public class BoersenmedienResolver extends AbstractWebSource {
    ...
    /** Direct-first - neither host walls (the old {@code @DirectFirst} seam). */
    @Inject
    public BoersenmedienResolver(WebFetcher fetcher) {
        super(fetcher);
    }

    @Override
    public String sourceName() {
        return "boersenmedien-resolver";
    }
    ...
    public List<Instrument> search(String query) {
        ...
        return parseSearch(fetchBody(url));
    }
    ...
    public Optional<Quote> quote(String isin) {
        ...
        String body = fetchBody(String.format(URL_SYMBOL_DA, key));
        if (body == null || parseQuote(body).isEmpty()) {
            body = fetchBody(String.format(URL_SYMBOL_BO, key));
        }
