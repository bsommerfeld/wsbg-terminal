 ...
 * The Origin/Referer recipe of the company-data client is NOT needed here.
 *
 ...
 *
 * <p>US symbol shapes only: a suffixed ({@code RHM.DE}), caret, future or
 * crypto symbol returns empty with ZERO network. Per-symbol 10-minute
 * politeness cache, misses cached too; a non-RSS 200 (bot-wall shell) is a
 * miss that is never cached, and an outage keeps the stale pool.
 *
 * <p>Transport {@code DIRECT,BROWSER}: the feed has no wall (probed
 * 2026-07-14), so the direct leg leads and the browser joker stays the
 * fallback seam.
 */
@Singleton
public class NasdaqNewsRssSource extends AbstractWebSource implements InstrumentSource {

    private static final Logger LOG = LoggerFactory.getLogger(NasdaqNewsRssSource.class);
    ...
    private record CachedFeed(Instant fetchedAt, List<Article> items) {}

    @Inject
    public NasdaqNewsRssSource(WebFetcher fetcher) {
        super(fetcher);
    }
    ...
    /**
     * The feed is symbol-addressed only — an instrument without a resolved
     * ticker answers empty, and the US-shape gate keeps every non-US symbol
     * off the network.
     */
    @Override
    public List<Article> newsFor(ResolvedInstrument instrument, int limit) {
        ...
        try {
            WebResponse resp = get(FEED_URL + sym,
    ...
    private static List<Article> stale(CachedFeed cached) {
    ...
    /**
     * RSS items → {@link Article}s. {@code <nasdaq:tickers>} is matched by
                ...
                while (r.hasNext()) {
                    ...
                    if (event == XMLStreamConstants.START_ELEMENT) {
                        ...
                        if ("item".equals(r.getLocalName())) {
                            inItem = false;
                            Article item = toItem(title, link, guid, pubDate,
    ...
    private static Article toItem(String title, String link, String guid, String pubDate,
