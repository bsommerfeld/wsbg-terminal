...
const DICT = {
  de: {
    ...
    'settings.headlines.images.hint': 'Wertet Bilder aus Threads und Kommentaren per Vision-Modell aus und lässt sie in Schlagzeilen einfließen. Aus = schnellere, reine Text-Schlagzeilen ohne Bildanalyse.',
    'settings.headlines.redund.name': 'Wiederholungen filtern',
    'settings.headlines.redund.hint': 'An (Standard): wiederholt sich eine Story ohne echte Neuigkeit, schweigt der Wire statt eine Zeile umzuformulieren. Aus = strikte 1:1-Spiegelung: jedes Signal schreibt eine Zeile, auch doppelt. (Die erste Zeile eines Subjekts kommt immer.)',
    ...
    'settings.updates.auto.hint': 'Aktualisiert beim Start automatisch. Wenn aus, meldet ein Hinweis in der Titelleiste neue Versionen.',
    ...
    'settings.data.clear.hint': 'Setzt das Terminal komplett zurück: Threads, Cluster, Schlagzeilen und das Archiv. Danach füllt sich die Wire vom nächsten Scan neu. Höchstens alle 10 Minuten.',
  ...
  en: {
    ...
    'settings.headlines.images.hint': 'Analyzes images from threads and comments with a vision model and feeds them into headlines. Off = faster, text-only headlines without image analysis.',
    'settings.headlines.redund.name': 'Filter repeats',
    'settings.headlines.redund.hint': "On (default): when a story repeats without real news, the wire stays silent instead of rephrasing a line. Off = strict 1:1 mirror: every signal writes a line, even a duplicate. (A subject's first line always comes through.)",
    ...
    'settings.updates.auto.hint': 'Updates automatically on start. When off, a hint in the title bar announces new versions.',
    ...
    'settings.data.clear.hint': 'Fully resets the terminal: threads, clusters, headlines and the archive. The wire then refills from the next scan. At most once every 10 minutes.',
