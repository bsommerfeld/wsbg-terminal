...
class MarketPressCatalogSmokeIT {
    ...
    /** The real house transport, direct-only - the sweep rides plain HTTP as before. */
    private static final de.bsommerfeld.wsbg.terminal.web.fetch.WebFetcher LIVE =
            new de.bsommerfeld.wsbg.terminal.web.impl.net.HouseFetcher(java.util.Set.of(
                    new de.bsommerfeld.wsbg.terminal.web.impl.net.DirectTransport()));

    @Test
    void everyCatalogedFeedStillAnswers() {
        List<MarketPressClient.PressHeadline> items =
                new MarketPressClient(LIVE).headlinesSince(
    ...
    private static String diagnose(MarketPressClient.Feed feed) {
        try {
            var resp = new de.bsommerfeld.wsbg.terminal.web.impl.net.DirectTransport().fetch(
                    feed.url(),
                    Map.of("Accept",
